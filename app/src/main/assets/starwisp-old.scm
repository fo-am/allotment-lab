...
(define-activity-list
 ...
 ;      (update-widget 'linear-layout (get-id "out-cost") 'hide 0)
   ...
   "field"
    (vert
     (medit-text 'field-name "normal" 
		 (lambda (v) (entity-update-single-value! (ktv "name" "varchar" v)) '()))
    ...
    (lambda (activity arg)
      ...
      (list
       ...
       ;;(update-widget 'text-view (get-id "field-title") 'text (entity-get-value "name"))
  ...
  (activity
   "fieldcalc"
   (vert
    (mtitle 'fieldcalc-title)
    (mtext 'fieldcalc-blurb)
    (horiz
	...
	(list (date-picker-dialog
		 ...
		 (let ((date (list day (+ month 1) year)))
		   (entity-update-single-value! 
		    (ktv "date" "varchar" (date->string date)))
		   (let ((season (date->season date)))
		     (entity-update-single-value! (ktv "season" "varchar" season))
		     (update-season! "fc" season)) 
		   (list
    ...
    (horiz
     (vert
      (mspinner 'manure-type manure-types-list
		(lambda (v)
		  (update-seek-mul! v)
		  (append
		   (update-type! "fc" v)
...
;;             (lambda () (list (start-activity "camera" 2 ""))))
   ...
   (lambda (activity arg)
     ...
     (set-current! 'field-values (get-entity-by-unique db "farm" (get-current 'field-id)))

     (mutate-state!
      (lambda (s)
        (state-modify-date s (list date-day date-month date-year))))

     (update-soil! "fc" (field-soil))
     (update-crop! "fc" (field-crop))
